...
function renderDetails(targetProduct, container) {
    container.innerHTML = `
    <div class="product-detail-container" data-product-id="${targetProduct.id}">
      <div class="product-detail-img-wrapper">
        ...
        <div class="product-detail-multi-img"></div>
      </div>
  ...
  `;
    const imgArr = targetProduct.images;
    imgArr.forEach((item) => {
        const image = document.createElement("img");
        image.src = item.url;
        document.querySelector(".product-detail-multi-img").append(image);
    });
    const img = container.querySelector(".product-detail-img");
...
if (targetProduct) {
    renderDetails(targetProduct, productDetailContainer);
} else {
    productDetailContainer.innerHTML = `<p>Không tìm thấy sản phẩm</p>`;
}

function updateMainImage(e) {
    document.querySelector(".product-detail-img").src = e.target.src;
}

document
    .querySelector(".product-detail-multi-img")
...
productDetailContainer.addEventListener("click", (e) => {
    ...
    const quantityValue =
        productDetailContainer.querySelector(".quantity-value");
